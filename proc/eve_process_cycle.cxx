...
//函数声明
static void eve_start_worker_processes(int threadnums);
static int eve_spawn_process(int threadnums,const char *pprocname);
static void eve_worker_process_cycle(int inum,const char *pprocname);
static void eve_worker_process_init(int inum);
...
{    
    ...
    {        
        eve_log_error_core(EVE_LOG_ALERT,errno,"eve_master_process_cycle()中sigprocmask()失败!");
    }
    ...
    if(size < 1000) //长度小于这个，我才设置标题
    {
        ...
        eve_setproctitle(title); //设置标题
        eve_log_error_core(EVE_LOG_NOTICE,0,"%s %P 【master进程】启动并开始运行......!",title,eve_pid); //设置标题时顺便记录下来进程名，进程id等信息到日志
    }    

    CConfig *p_config = CConfig::GetInstance(); //单例类
    int workprocess = p_config->GetIntDefault("WorkerProcesses",1); //从配置文件中得到要创建的worker进程数量
    eve_start_worker_processes(workprocess);  //这里要创建worker子进程
...
static void eve_start_worker_processes(int threadnums)
{
    ...
    {
        eve_spawn_process(i,"worker process");
    } //end for
    return;
}
...
static int eve_spawn_process(int inum,const char *pprocname)
{
    pid_t  pid;
    ...
    {  
    case -1: //产生子进程失败
        eve_log_error_core(EVE_LOG_ALERT,errno,"eve_spawn_process()fork()产生子进程num=%d,procname=\"%s\"失败!",inum,pprocname);
        return -1;

    case 0:  //子进程分支
        eve_parent = eve_pid;              //因为是子进程了，所有原来的pid变成了父pid
        eve_pid = getpid();                //重新获取pid,即本子进程的pid
        eve_worker_process_cycle(inum,pprocname);    //我希望所有worker子进程，在这个函数里不断循环着不出来，也就是说，子进程流程不往下边走;
        break;
...
{
    //设置一下变量
    eve_process = EVE_PROCESS_WORKER;  //设置进程的类型，是worker进程

    //重新为子进程设置进程名，不要与父进程重复------
    eve_worker_process_init(inum);
    eve_setproctitle(pprocname); //设置标题   
    eve_log_error_core(EVE_LOG_NOTICE,0,"%s %P 【worker进程】启动并开始运行......!",pprocname,eve_pid); //设置标题时顺便记录下来进程名，进程id等信息到日志

    for(;;)
    {  
        eve_process_events_and_timers(); //处理网络事件和定时器事件
...
static void eve_worker_process_init(int inum)
{
    ...
    {
        eve_log_error_core(EVE_LOG_ALERT,errno,"eve_worker_process_init()中sigprocmask()失败!");
    }
    ...
    g_socket.eve_epoll_init();           //初始化epoll相关内容，同时 往监听socket上增加监听事件，从而开始让监听端口履行其职责
