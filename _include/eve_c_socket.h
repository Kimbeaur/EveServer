...
#ifndef __EVE_SOCKET_H__
#define __EVE_SOCKET_H__
...
//一些宏定义放在这里-----------------------------------------------------------
#define eve_LISTEN_BACKLOG  511    //已完成连接队列，
#define eve_MAX_EVENTS      512    //epoll_wait一次最多接收这么多个事件

typedef struct eve_listening_s   eve_listening_t;
typedef struct eve_connection_s  eve_connection_t;
typedef class  CSocket           CSocket;

typedef void (CSocket::*eve_event_handler_pt)(eve_connection_t* c); //定义成员函数指针

//--------------------------------------------
//一些专用结构定义放在这里，暂时不考虑放eve_global.h里了
struct eve_listening_s  //和监听端口有关的结构
{
	int                       port;        //监听的端口号
	int                       fd;          //套接字句柄socket
	eve_connection_t*        connection;  //连接池中的一个连接，注意这是个指针 
};


//该结构表示一个TCP连接【客户端主动发起的、Nginx服务器被动接受的TCP连接】
struct eve_connection_s
{		
	eve_connection_s();                                      //构造函数
	virtual ~eve_connection_s();                             //析构函数
	void GetOneToUse();                                      //分配出去的时候初始化一些内容
	void PutOneToFree();                                     //回收回来的时候做一些事情


	int                       fd;                            //套接字句柄socket
	eve_listening_t*         listening;                     //如果这个链接被分配给了一个监听套接字，那么这个里边就指向监听套接字对应的那个eve_listening_t*的内存首地址		

	uint64_t                  iCurrsequence;                 //我引入的一个序号，每次分配出去时+1，此法也有可能在一定程度上检测错包废包，具体怎么用，用到了再说
	struct sockaddr           s_sockaddr;                    //保存对方地址信息用的

	eve_event_handler_pt      rhandler;                       //读事件的相关处理方法
	eve_event_handler_pt      whandler;                       //写事件的相关处理方法
	...
	//--------------------------------------------------
	eve_connection_t*        next;                           //这是个指针，指向下一个本类型对象，用于把空闲的连接池对象串起来构成一个单向链表，方便取用
};

//消息头，引入的目的是当收到数据包时，额外记录一些内容以备将来使用
typedef struct _msg_head_t
{
	eve_connection_t* pConn;         //记录对应的链接，注意这是个指针
...
public:
	...
	virtual void procPingTimeOutChecking(msg_head_t* tmpmsg,time_t cur_time);  //心跳包检测时间到，该去检测心跳包是否超时的事宜，本函数只是把内存释放，子类应该重新事先该函数以实现具体的判断动作

public:	
	int  eve_epoll_init();                                                //epoll功能初始化	
	int  eve_epoll_process_events(int timer);                             //epoll等待接收和处理事件

	int eve_epoll_oper_event(int fd,uint32_t eventtype,uint32_t flag,int bcaction,eve_connection_t* pConn); 
	                                                                      //epoll操作事件
	
protected:
	//数据发送相关
	void msgSend(char *psendbuf);                                         //把数据扔到待发送对列中 
	void zdClosesocketProc(eve_connection_t* p_Conn);                    //主动关闭一个连接时的要做些善后的处理函数	
	
private:	
	void ReadConf();                                                      //专门用于读各种配置项	
	bool eve_open_listening_sockets();                                    //监听必须的端口【支持多个端口】
	void eve_close_listening_sockets();                                   //关闭监听套接字
	bool setnonblocking(int sockfd);                                      //设置非阻塞套接字	

	//一些业务处理函数handler
	void eve_event_accept(eve_connection_t* oldc);                       //建立新连接
	void eve_read_request_handler(eve_connection_t* pConn);              //设置数据来时的读处理函数
	void eve_write_request_handler(eve_connection_t* pConn);             //设置数据发送时的写处理函数
	void eve_close_connection(eve_connection_t* pConn);                  //通用连接关闭函数，资源用这个函数释放【因为这里涉及到好几个要释放的资源，所以写成函数】

	ssize_t recvproc(eve_connection_t* pConn,char *buff,ssize_t buflen); //接收从客户端来的数据专用函数
	void eve_wait_request_handler_proc_p1(eve_connection_t* pConn,bool &isflood); 
	                                                                      //包头收完整后的处理，我们称为包处理阶段1：写成函数，方便复用      
	void eve_wait_request_handler_proc_plast(eve_connection_t* pConn,bool &isflood);   
	                                                                      //收到一个完整包后的处理，放到一个函数中，方便调用	
	void clearMsgSendQueue();                                             //处理发送消息队列  

	ssize_t sendproc(eve_connection_t* c,char *buff,ssize_t size);       //将数据发送到客户端 

	//获取对端信息相关                                              
	size_t eve_sock_ntop(struct sockaddr *sa,int port,u_char *text,size_t len);  //根据参数1给定的信息，获取地址端口字符串，返回这个字符串的长度

	//连接池 或 连接 相关
	void initconnection();                                                //初始化连接池
	void clearconnection();                                               //回收连接池
	eve_connection_t* eve_get_connection(int isock);                     //从连接池中获取一个空闲连接
	void eve_free_connection(eve_connection_t* pConn);                   //归还参数pConn所代表的连接到到连接池中	
	void inRecyConnectQueue(eve_connection_t* pConn);                    //将要回收的连接放到一个队列中来
	
	//和时间相关的函数
	void    AddToTimerQueue(eve_connection_t* pConn);                    //设置踢出时钟(向map表中增加内容)
	time_t  GetEarliestTime();                                            //从multimap中取得最早的时间返回去
	msg_head_t* RemoveFirstTimer();                                //从m_timeQueuemap移除最早的时间，并把最早这个时间所在的项的值所对应的指针 返回，调用者负责互斥，所以本函数不用互斥，
	msg_head_t* GetOverTimeTimer(time_t cur_time);                  //根据给的当前时间，从m_timeQueuemap找到比这个时间更老（更早）的节点【1个】返回去，这些节点都是时间超过了，要处理的节点      
	void DeleteFromTimerQueue(eve_connection_t* pConn);                  //把指定用户tcp连接从timer表中抠出去
	void clearAllFromTimerQueue();                                        //清理时间队列中所有内容

	//和网络安全有关
	bool TestFlood(eve_connection_t* pConn);                             //测试是否flood攻击成立，成立则返回true，否则返回false
...
protected:
	//一些和网络通讯有关的成员变量
	size_t                         m_iLenPkgHeader;                       //sizeof(comm_pkg_head_t);		
	size_t                         m_iLenMsgHeader;                       //sizeof(msg_head_t);
...
private:
	...
	//和连接池有关的
	std::list<eve_connection_t*>  m_connectionList;                      //连接列表【连接池】
	std::list<eve_connection_t*>  m_freeconnectionList;                  //空闲连接列表【这里边装的全是空闲的连接】
	...
	std::list<eve_connection_t*>  m_recyconnectionList;                  //将要释放的连接放这里
	std::atomic<int>               m_totol_recyconnection_n;              //待释放连接队列大小
	int                            m_RecyConnectionWaitTime;              //等待这么些秒后才回收连接
	
	std::vector<eve_listening_t*> m_ListenSocketList;                    //监听套接字队列
	struct epoll_event             m_events[eve_MAX_EVENTS];              //用于在epoll_wait()中承载返回的所发生的事件
	...
	std::multimap<time_t, msg_head_t*>   m_timerQueuemap;          //时间队列	
