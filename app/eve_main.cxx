...
#include "eve_macro.h"         //各种宏定义
#include "eve_func.h"          //各种函数声明
#include "eve_c_conf.h"        //和配置文件处理相关的类,名字带c_表示和类有关
#include "eve_c_socket.h"      //和socket通讯相关
#include "eve_c_memory.h"      //和内存分配释放等相关
#include "eve_c_threadpool.h"  //和多线程有关
#include "eve_c_crc32.h"       //和crc32校验算法有关 
#include "eve_c_slogic.h"      //和socket通讯相关
...
char    *gp_envmem=NULL;        //指向自己分配的env环境变量的内存，在eve_init_setproctitle()函数中会被分配内存
...
//和进程本身有关的全局量
pid_t   eve_pid;                //当前进程的pid
pid_t   eve_parent;             //父进程的pid
int     eve_process;            //进程类型，比如master,worker进程等
int     g_stopEvent;            //标志程序退出,0不退出1，退出

sig_atomic_t  eve_reap;         //标记子进程状态变化[一般是子进程发来SIGCHLD信号表示退出],sig_atomic_t:系统定义的类型：访问或改变这些变量需要在计算机的一条指令内完成
...
{     
    ...
    //(1)无伤大雅也不需要释放的放最上边    
    eve_pid    = getpid();      //取得进程pid
    eve_parent = getppid();     //取得父进程的id 
    //统计argv所占的内存
    g_argvneedmem = 0;
    ...
    //全局量有必要初始化的
    eve_log.fd = -1;                  
    eve_process = EVE_PROCESS_MASTER; //先标记本进程是master进程
    eve_reap = 0;                     //标记子进程没有发生变化
   
    //(2)初始化失败，就要直接退出的
    //配置文件必须最先要，后边初始化啥的都用，所以先把配置读出来，供后续使用 
    CConfig *p_config = CConfig::GetInstance(); //单例类
    if(p_config->Load("Server.conf") == false) //把配置文件内容载入到内存            
    {   
        eve_log_init();    //初始化日志
        eve_log_stderr(0,"配置文件[%s]载入失败，退出!","nginx.conf");
    ...
    //(3)一些必须事先准备好的资源，先初始化
    eve_log_init();             //日志初始化(创建/打开日志文件)，这个需要配置项，所以必须放配置文件载入的后边；     
        
    //(4)一些初始化函数，准备放这里        
    if(eve_init_signals() != 0) //信号初始化
    {
        exitcode = 1;
        goto lblexit;
    }        
    ...
    //(7)开始正式的主工作流程，主流程一致在下边这个函数里循环，暂时不会走下来   
    eve_master_process_cycle(); //不管父进程还是子进程，正常工作期间都在这个函数里循环；
        
 
lblexit:
    //(5)该释放的资源要释放掉
    eve_log_stderr(0,"程序退出，再见了!");
...
void freeresource()
{
    ...
    //(2)关闭日志文件
    if(eve_log.fd != STDERR_FILENO && eve_log.fd != -1)  
    {        
        close(eve_log.fd); //不用判断结果了
        eve_log.fd = -1; //标记下，防止被再次close吧        
    }
}
