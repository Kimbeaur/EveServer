...
#SRCS = nginx.c eve_conf.c
SRCS = $(wildcard *.cxx)

#OBJS = nginx.o eve_conf.o  这么一个一个增加.o太麻烦，下行换一种写法：把字符串中的.c替换为.o
OBJS = $(SRCS:.cxx=.o)

#把字符串中的.c替换为.d
#DEPS = nginx.d eve_conf.d
DEPS = $(SRCS:.cxx=.d)
...
#处理后形如 /mnt/hgfs/linux/nginx/app/link_obj/eve_signal2.o /mnt/hgfs/linux/nginx/app/link_obj/eve_signal.o
# := 在解析阶段直接赋值常量字符串【立即展开】，而 = 在运行阶段，实际使用变量时再进行求值【延迟展开】
# /mnt/hgfs/linux/nginx/app/link_obj/nginx.o   /mnt/hgfs/linux/nginx/app/link_obj/eve_conf.o 
...
#这里是诸多.d文件被包含进来，每个.d文件里都记录着一个.o文件所依赖哪些.c和.h文件。内容诸如 nginx.o: nginx.c eve_func.h
...
#.d文件中的内容可能形如：nginx.o: nginx.c eve_func.h
#%.d:%.c
$(DEP_DIR)/%.d:%.cxx
#gcc -MM $^ > $@
#.d文件中的内容形如：nginx.o: nginx.c eve_func.h ../signal/eve_signal.h，但现在的问题是我们的.o文件已经放到了专门的目录
...
#上行处理后，.d文件中内容应该就如：/mnt/hgfs/linux/nginx/app/link_obj/nginx.o: nginx.c eve_func.h ../signal/eve_signal.h
